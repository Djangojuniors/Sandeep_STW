    ...
    if BE.pages() != 0:
        for _ in range(_pages):
            ...
            if (df is not None) and (df1 is not None):
                ...
                plan, year, half_year, quarter, month = 0, 0, 0, 0, 0
                ...
                try:
                    ...
                    if len(yearly_df) != 0 and len(half_yearly_df) != 0 and len(quarterly_df) != 0 and len(
                            monthly_df) != 0:
                        ...
                        if len(plan_df1) > 0:
                            plan_df['Plan Type'] = df1[plan]
                            plan_df['Plan Type'].replace('', np.nan, inplace=True)
                            plan_df.dropna(subset=['Plan Type'], inplace=True)
