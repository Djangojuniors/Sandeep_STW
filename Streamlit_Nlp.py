...
KEY_WORDS = ['Monthly', 'Quarterly', 'Half Yearly', 'Annually', 'Total', 'Half-Yearly', 'Accounts', 'Plan', 'Term',
             'Sum Assured',
             'Premium', 'Age', 'Sex', 'Gender', 'Coverage Type', 'Rider']

CLIENT_DETAIL = ['Name', 'Sex', 'Smoker', 'Premium Mode', 'Age', 'Occupation Class', 'Basic Sum Assured',
...
#     with pdfplumber(self.pdf_name) as _pdf:
...
#     # _tables = tabula.read_pdf(_pdf_name), pages=page_number, multiple_tables=True)
#     _tables = tabula.read_pdf(_pdf_name, pages = page_number, multiple_tables= True)
#     # print(_tables)
#     return _tables

def norm_pdf_page(pg):
    uploaded_file = st.file_uploader("Choose a file", "pdf")
    pdf_doc = PdfFileReader(open(uploaded_file, "rb"))
